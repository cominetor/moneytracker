...
- **Prezzi azioni** da [Alpha Vantage](https://www.alphavantage.co/) (`GLOBAL_QUOTE`, in
  USD), convertiti in EUR con il cambio di [Frankfurter](https://frankfurter.app/)
  (gratuito, senza key).
...
- **Impostazioni**: API key Alpha Vantage (con link alla pagina per ottenerla gratis) e
  intervallo di refresh dei prezzi (default 15 minuti).
...
src/
  ...
  services/                layer API + persistenza (nessuna dipendenza dalla UI)
    ...
    alphaVantage.js        prezzi azioni (GLOBAL_QUOTE)
    rateLimiter.js         coda/throttle 5 al minuto, 25 al giorno
    fx.js                  cambio USD → EUR (Frankfurter)
